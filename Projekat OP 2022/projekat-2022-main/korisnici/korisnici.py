...
def kreiraj_korisnika(svi_korisnici: dict, azuriraj: bool, uloga: str, staro_korisnicko_ime: str, 
    ...
    korisnik = [uloga, korisnicko_ime, lozinka, ime, prezime, email]
    ...
    # korisnicko_ime
    if not isinstance(korisnicko_ime, str):
        raise Exception("Greška! Korisničko ime mora biti string!")
    # lozinka
    if not isinstance(lozinka, str):
    ...
    # drzavljanstvo
    if len(drzavljanstvo) > 0:
        if not isinstance(drzavljanstvo, str):
            raise Exception("Greška! Državljanstvo mora biti string!")
    # telefon
    if len(telefon) > 0 and not telefon.isnumeric():
...
def logout(korisnicko_ime: str):
    print(f"\nUpravo ste se odjavili {korisnicko_ime}. Možete nesmetano nastaviti da koristite aplikaciju sa ograničenim opcijama.\n")
